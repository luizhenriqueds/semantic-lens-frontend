...
import { getProperties, getRegion, getRegions } from "@/lib/data";
...
export default async function RegionPage({ params }: { params: Promise<{ h3: string }> }) {
  ...
  const [region, all] = await Promise.all([getRegion(h3), getProperties()]);
  if (!region) notFound();
      ...
      <div className="rgrid" style={{ gridTemplateColumns: "1fr 1fr" }}>
        ...
        <div className="rcard">
          ...
          <div className="rcbody">
            <p className="rabout">
              {region.resumo ??
                `${region.nome}, em ${region.cidade}, com ${region.numProps} imóveis analisados.`}
            </p>
          </div>
        </div>
      </div>
