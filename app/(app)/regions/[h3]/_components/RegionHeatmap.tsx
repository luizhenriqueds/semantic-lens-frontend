...
export default function RegionHeatmap({ region }: { region: Region }) {
  const spots = [
    { x: 50, y: 52, r: 170, i: 0.52, name: region.name, hot: true },
    { x: 24, y: 30, r: 124, i: 0.3, name: region.neighbors[0]?.name ?? "Arredores" },
    { x: 78, y: 28, r: 128, i: 0.28, name: region.neighbors[1]?.name ?? "Arredores" },
    { x: 27, y: 80, r: 108, i: 0.2, name: region.neighbors[2]?.name ?? "Arredores" },
